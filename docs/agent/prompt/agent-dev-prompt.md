...
4. **Linked PR and AgentReview feedback (if any):** Determine whether a PR already exists for this work (issue timeline comments, `gh pr list --head <implementation_branch>`, or GitHub API). **If a PR exists:** load **pull request reviews** (treat **Request changes** / `CHANGES_REQUESTED` as highest priority), **inline review comments**, and any unresolved threads. **Address that feedback in code first**—implement fixes, commit, and push to `implementation_branch`. Do not ignore review feedback to add unrelated scope. **If no PR exists yet:** continue with implementation per the plan below. **Note:** [`router.yml`](../../.github/workflows/router.yml) does **not** re-invoke this automation on push alone; humans typically **toggle** issue label **`status:in_progress`** (remove, then add) to run AgentDev again after AgentReview.
5. Implement the plan: edit application code and config as needed (including any remaining plan items after review fixes). Follow repository conventions. Do not disable or weaken CI, Sonar, or Snyk checks.
6. Run the same checks locally if available (`npm run lint`, `npm run build`, etc.) and fix straightforward failures.
7. **Commit** on the **same** implementation branch with clear messages. **Every commit must link to the issue** so GitHub shows each commit on the issue timeline:
...
9. **Open or update PR:** **If a PR already exists** from earlier work, do **not** open a second PR—your pushes update it. **If no PR exists yet**, open a **pull request** from `implementation_branch` into `main` using `.github/pull_request_template.md`. If you use **Cursor’s “Open Pull Request” (GitHub) tool**, GitHub MCP, or the API, set the PR **head branch** explicitly to **`implementation_branch`** (the name from the plan file and `git push`). Do **not** rely on the **current checkout** if it is a Cursor automation branch (names like `cursor/issue-plan-development-…` are often **not** on `origin` and cause “branch not pushed” errors even when the real branch exists). Prefer **`gh pr create`** with `--head <implementation_branch>` if a GUI tool keeps picking the wrong branch. **You must link the PR to the issue** in all of the following ways:
   ...
   - **After the PR exists:** **Comment on the GitHub issue** with the PR number and full URL, e.g. `Implementation PR: #<pr_number> — https://github.com/<owner>/<repo>/pull/<pr_number>`. This appears in the issue timeline and cements the link for humans and automations (skip duplicate issue comment if unchanged PR already linked).
10. On the **PR**: ensure label `agent:review` is present so the review automation can run (add if missing). Do **not** merge.
11. On the **issue**: update labels toward implementation (e.g. `status:implementing` / `status:pr_open` per team convention) and leave assignee/comment as appropriate.
